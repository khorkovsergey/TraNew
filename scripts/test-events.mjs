...
try {
  execFileSync(
    ...
    [
      ...
      'src/lib/voyager/tools/container.ts',
  ...
  const containers = await load('container', 'tools');
  ...
  group('A code-execution container is carried through one answer');

  check('a container the API named is adopted', () => {
    /*
     * From production: `400 invalid_request_error — container_id is required
     * when there are pending tool uses generated by code execution with tools`.
     * Web search filters its results inside a container, so a round that
     * searched leaves pending work the next round has to name.
     */
    assert.equal(
      containers.nextContainer(undefined, { id: 'cntr_abc123', expires_at: 'later' }),
      'cntr_abc123'
    );
  });

  check('nothing is sent until there is something to send', () => {
    // A conversation that never searched sends exactly what it sent before this
    // existed.
    assert.deepEqual(containers.containerParam(undefined), {});
    assert.deepEqual(containers.containerParam('cntr_abc123'), { container: 'cntr_abc123' });
  });

  check('a round that mentions no container keeps the one in hand', () => {
    /*
     * The failure mode this prevents: dropping it on the round that did not
     * touch code execution, and reproducing the 400 on the round after.
     */
    for (const absent of [null, undefined, {}, { id: null }, { id: '' }, { id: 42 }]) {
      assert.equal(
        containers.nextContainer('cntr_abc123', absent),
        'cntr_abc123',
  ...
  check('and a container is never invented', () => {
    for (const absent of [null, undefined, {}, { id: '   ' }, { id: 7 }]) {
      assert.equal(containers.nextContainer(undefined, absent), undefined, JSON.stringify(absent));
    }
  });

  check('a later container replaces the earlier one', () => {
    // The API is the only thing entitled to name one.
    assert.equal(
      containers.nextContainer('cntr_first', { id: 'cntr_second', expires_at: 'later' }),
      'cntr_second'
    );
  });

  check('the sequence that failed in production now continues', () => {
    // response 1 -> container -> response 2 carries the same one -> the final
    // pass still carries it.
    let held;
    held = containers.nextContainer(held, { id: 'cntr_run1', expires_at: 'later' });
    assert.deepEqual(containers.containerParam(held), { container: 'cntr_run1' });

    held = containers.nextContainer(held, null); // a tool round, no container
    assert.deepEqual(containers.containerParam(held), { container: 'cntr_run1' });

    held = containers.nextContainer(held, undefined); // the final pass
    assert.deepEqual(containers.containerParam(held), { container: 'cntr_run1' });
  });

  check('two answers cannot share one, by construction', () => {
    /*
     * The container is a local inside one `askVoyager` execution, so it cannot
     * outlive the answer, reach storage, or belong to somebody else's question.
     * Asserted on the source because that scoping *is* the guarantee.
     */
    const source = readFileSync('src/lib/voyager/orchestrator.ts', 'utf8');
    const declaration = source.indexOf('let container: string | undefined;');
    const entry = source.indexOf('export async function askVoyager');
    assert.ok(declaration > 0, 'no request-local container');
    assert.ok(declaration > entry, 'the container outlives one answer');

    const module_ = readFileSync('src/lib/voyager/tools/container.ts', 'utf8');
    assert.ok(!/localStorage|sessionStorage|db\.|schema\./.test(module_));
  });

  check('and it is sent on every request after the one that made it', () => {
    const source = readFileSync('src/lib/voyager/orchestrator.ts', 'utf8');
    // One create call, one spread — the tool round, the resume and the final
    // pass all go through it.
    assert.ok(source.includes('...containerParam(container)'));
    assert.ok(source.includes('container = nextContainer(container, response.container)'));
  });

  check('a rejected request is not reported as an unreachable model', () => {
    /*
     * The 400 spent a week looking like an outage in the logs, which is where
     * the container defect hid. No retry is added for it — the API marks it
     * non-retryable, and the same body would be refused identically.
     */
    const source = readFileSync('src/lib/voyager/orchestrator.ts', 'utf8');
    assert.ok(source.includes('instanceof Anthropic.APIError'));
    assert.ok(source.includes('the request I built was not one the model would accept'));
  });

  group('Comparing instruments is a different screen from exploring categories');

  check('"where can I compare assets" reaches the comparison screen', () => {
    /*
     * It reached Explore, which is where somebody learns what a bond is. A
     * person asking where to compare assets wants tickers side by side.
     */
    const found = nav.findDestinations('compare_assets', acts.VOYAGER_ACTION_IDS);
    assert.equal(found.ok, true);
    assert.equal(found.data.destinations[0].action, 'open_market_compare');
  });

  check('and exploring categories still reaches Explore', () => {
    const found = nav.findDestinations('explore_categories', acts.VOYAGER_ACTION_IDS);
    assert.equal(found.ok, true);
    assert.deepEqual(
      found.data.destinations.map((d) => d.action),
      ['open_explore']
    );
  });

  check('the two intents never resolve to each other', () => {
    const compare = nav
      .findDestinations('compare_assets', acts.VOYAGER_ACTION_IDS)
      .data.destinations.map((d) => d.action);
    const explore = nav
      .findDestinations('explore_categories', acts.VOYAGER_ACTION_IDS)
      .data.destinations.map((d) => d.action);

    assert.ok(!compare.includes('open_explore'), compare.join(','));
    assert.ok(!explore.includes('open_market_compare'), explore.join(','));
  });

  check('the action is in the registry and offered where it makes sense', () => {
    assert.equal(acts.isVoyagerActionId('open_market_compare'), true);
    const spec = acts.specFor('open_market_compare');
    assert.equal(spec.execution, 'navigate');
    assert.ok(
      acts
        .allowedActions({ screen: 'generic', tier: 'basic', hasTicker: false })
        .includes('open_market_compare')
    );
  });

  check('and the portal says which is which', () => {
    /* Its own menu rows: the shared fixture is declared further down, and a
       test that reaches forward for one is a test that breaks when it moves. */
    const rows = [{ label: 'Compare assets', kind: 'route' }];
    const byId = Object.fromEntries(portalKnowledge.portalSections(rows).map((s) => [s.id, s]));
    assert.equal(byId.compare.action, 'open_market_compare');
    assert.equal(byId.explore.action, 'open_explore');
    assert.match(byId.compare.notToBeConfusedWith, /Explore/);
    assert.match(byId.explore.notToBeConfusedWith, /Compare assets/);
  });
